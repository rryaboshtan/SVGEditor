#!/usr/bin/env python3
"""Register aspect/svelte/rn SEO tool pages in build.sh and sitemap.xml."""
...
slugs = [
    s.strip()
    for s in (root / "scripts" / "_asr_slugs.txt").read_text(encoding="utf-8").splitlines()
    if s.strip()
]
...
marker = '    "remove-unused-defs-from-svg.html",\n'
insert = "".join(f'    "{s}.html",\n' for s in slugs)
if any(f'"{s}.html"' in text for s in slugs[:3]):
    print("build.sh already has ASR pages")
else:
...
if not new_slugs:
    ...
    for s in new_slugs:
        entries.append(
            ...
            "    <lastmod>2026-09-08T01:44:26+03:00</lastmod>\n"
