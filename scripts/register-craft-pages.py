#!/usr/bin/env python3
"""Register craft long-tail HTML pages in build.sh and sitemap.xml."""
...
slugs = [
    s.strip()
    for s in (root / "scripts" / "_craft_slugs.txt").read_text(encoding="utf-8").splitlines()
    if s.strip()
]

build = root / "build.sh"
text = build.read_text(encoding="utf-8")
marker = '    "svg-to-react-native-forwardref-component.html",\n'
insert = "".join(f'    "{s}.html",\n' for s in slugs)
if any(f'"{s}.html"' in text for s in slugs[:3]):
    print("build.sh already has craft pages")
else:
